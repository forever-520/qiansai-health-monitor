...
LUBANCAT_HOST="${LUBANCAT_HOST:-rk3576-slave.local}"
LUBANCAT_PORT="${LUBANCAT_PORT:-9001}"
...
export LUBANCAT_HOST LUBANCAT_PORT REMOTE_RADAR_URL REMOTE_CAMERA_CAPTURE_URL
...
python3 "$ROOT/lubancat_rk_gateway.py" \
  --lubancat-host "$LUBANCAT_HOST" \
  --lubancat-port "$LUBANCAT_PORT" \
