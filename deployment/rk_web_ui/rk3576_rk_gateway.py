...
DISCOVERY_CACHE = Path(__file__).resolve().parent / "data" / "rk3576_slave_last_ip.txt"
DISCOVERY_HTTP_PORT = int(os.environ.get("RK3576_SLAVE_HTTP_PORT", "8000"))
DISCOVERY_PREFERRED_HOSTS = os.environ.get("RK3576_SLAVE_PREFERRED_HOSTS", "")
DISCOVERY_PREFERRED_TIMEOUT = float(os.environ.get("RK3576_SLAVE_PREFERRED_TIMEOUT", "8.0"))
DISCOVERY_SCAN_TIMEOUT = float(os.environ.get("RK3576_SLAVE_SCAN_TIMEOUT", "0.45"))
REMOTE_HTTP_TIMEOUT = float(os.environ.get("RK3576_SLAVE_POLL_TIMEOUT", "8.0"))
REMOTE_CAMERA_TIMEOUT = float(os.environ.get("RK3576_SLAVE_CAMERA_TIMEOUT", "35.0"))
...
def _probe_rk3576_slave_http(host, port=DISCOVERY_HTTP_PORT, timeout=0.45):
    try:
...
def discover_rk3576_slave_http_host(
    ...
    for host in _preferred_hosts(DISCOVERY_PREFERRED_HOSTS if preferred_hosts is None else preferred_hosts):
        if probe(host, http_port, DISCOVERY_PREFERRED_TIMEOUT):
            ...
            print(f"RK3576_SLAVE_DISCOVERY preferred {host}", flush=True)
            return host

    cached = _read_cached_host(cache_path)
    if cached and probe(cached, http_port, DISCOVERY_PREFERRED_TIMEOUT):
        print(f"RK3576_SLAVE_DISCOVERY cached {cached}", flush=True)
        return cached

    candidates = _candidate_hosts(networks or _local_ipv4_networks())
    if not candidates:
        print("RK3576_SLAVE_DISCOVERY no local IPv4 network found", flush=True)
        return ""

    print(f"RK3576_SLAVE_DISCOVERY scanning {len(candidates)} hosts on port {http_port}", flush=True)
    with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, max_workers)) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            if ok:
                ...
                print(f"RK3576_SLAVE_DISCOVERY found {candidate}", flush=True)
                return candidate

    print("RK3576_SLAVE_DISCOVERY not found", flush=True)
    return ""
...
class RemoteHttpClient:
    ...
    def _rediscover(self):
        if not self.auto_discover:
            return False
        host = discover_rk3576_slave_http_host("auto")
        if not host:
            return False
        ...
        while True:
            try:
                if not self.url and not self._rediscover():
                    raise RuntimeError("rk3576_slave not discovered")
...
def main():
    ...
    state = SharedState()
    client = None
    auto_http = is_auto_value(args.remote_url) or is_auto_value(args.rk3576_slave_host)
    if auto_http:
        host = discover_rk3576_slave_http_host("auto")
